...
export default function Header({ headers, setColumnOrder, onSort, sortConfig }: TableHeaderProps) {
  const onDragEnd = () => {
    let tmpHeaders = clone(headers)
    const _dragItem = tmpHeaders.splice(dragIndex, 1)[0]
    tmpHeaders.splice(enterIndex, 0, _dragItem)
    localStorage.setItem('promoColumnOrder', JSON.stringify(tmpHeaders))
